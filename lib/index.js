...
var content = fs.readFileSync(
    // path.join(__dirname, '..') + path.sep + 'test/all-test.less',
    ...
    // path.join(__dirname, '..') + path.sep + 'test/blank.less',
    path.join(__dirname, '..') + path.sep + 'test/selector1.less',
    'utf8'
);
...
lessParser.parse(
    ...
    function (err, tree) {
        if (err) {
            ...
            var lessAstJsonFile = path.join(__dirname, '..') + path.sep + 'less-ast.json';
        ...
        var jisonAstFile = path.join(__dirname, '..') + path.sep + 'jison-ast.json';
